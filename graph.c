...
void build_graph_cmd (pnode * head, char *input){
        ...
        if (*input == 110){
            ...
            while (scanf ("%d", &dst)){
                ...
                if ((*input >= 65 && *input <= 90) || *input != 110)
                    break;
                scanf ("%d", &w);
                pnode second = idFinder (head, dst);
...
void insert_node_cmd (pnode * head, char *choose){
...
void delete_node_cmd (pnode * head, char *choose){
...
void deleteGraph_cmd (pnode * head, char *input){
...
bool update (pnode src, pnode dst, int w) {
    pedge runEdge = src->edges;
    if (runEdge == NULL)
        return false;
    while (runEdge != NULL){
        if (runEdge->endpoint == dst){
            runEdge->w = w;
            return true;
        }
...
void createEdge (pnode src, pnode dst, int w){
...
int dijkstra (pnode src, pnode dst, pnode * src_pointer){
    ...
    free (head);
    return idFinder (src_pointer, dst->node_num)->w;
}
